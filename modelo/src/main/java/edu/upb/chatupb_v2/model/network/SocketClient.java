...
//                        acept.setIp(ip);
            ...
            if (!socket.isClosed()) {
                if (socketListener != null){
                    for (SocketListener listener : socketListener) {
                        EventQueue.invokeLater(() -> listener.onDisconnect(this));
                    }
                }
                e.printStackTrace();
    ...
    public void notificar(Message message) {
        for (SocketListener listener : socketListener) {
            EventQueue.invokeLater(() -> listener.onMessage(this, message));
        }
    }
    ...
    public void close(SocketListener listener) {
        try {
